 ...
 * <p>
 * In its present form it has one significant limitation.  It uses the {@link SVG#renderToPicture(int)}
...
public class SVGImageView extends ImageView
{
    ...
    */
   public void  setSVG(SVG mysvg, int angle)
   {
      if (mysvg == null)
         throw new IllegalArgumentException("Null value passed to setSVG()");

      setSoftwareLayerType();
      setImageDrawable(new PictureDrawable(mysvg.renderToPicture(angle)));
   }
   ...
   private SVGGetter svgGetter;
   public void setSVGGetter(SVGGetter svgGetter){
      this.svgGetter = svgGetter;
   }
   public interface  SVGGetter{
      void onSVGCreated(SVG svg);
   }
   ...
   private class LoadAssetTask extends AsyncTask<String, Integer, Picture>
   {
      ...
      protected Picture  doInBackground(String... params)
      {
         try
         {
            ...
            //List<SVG.SvgObject> svgObjects = svg.getSVGObject();
           //SVG.SvgObject svgObject = svgObjects.get(0);
           //Path  path = svgObject.document.get;
            if(svgGetter!=null){
               svgGetter.onSVGCreated(svg);
            }
            return svg.renderToPicture(-1);
         }
         catch (SVGParseException e)
