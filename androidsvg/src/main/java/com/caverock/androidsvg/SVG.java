...
public class SVG {
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public Picture renderToPicture(int angle) {
        ...
        if (width != null) {
            ...
            return renderToPicture((int) Math.ceil(w), (int) Math.ceil(h), angle);
        } else {
            return renderToPicture(DEFAULT_PICTURE_WIDTH, DEFAULT_PICTURE_HEIGHT, angle);
        }
    }
     ...
     */
    @SuppressWarnings({"WeakerAccess", "unused"})
    public Picture renderToPicture(int widthInPixels, int heightInPixels, int angle) {
        Picture picture = new Picture();
        Canvas canvas = picture.beginRecording(widthInPixels, heightInPixels);
        Box viewPort = new Box(0f, 0f, (float) widthInPixels, (float) heightInPixels);

        SVGAndroidRenderer renderer = new SVGAndroidRenderer(canvas, this.renderDPI, angle);
     ...
     * to the viewport.  In other words, use {@link #renderToPicture(int)} to render the whole document, or use this
    ...
    public Picture renderViewToPicture(String viewId, int widthInPixels, int heightInPixels) {
        ...
        SVGAndroidRenderer renderer = new SVGAndroidRenderer(canvas, this.renderDPI, -1);
   ...
   */
    ...
    public List<SvgObject> getSVGObject(){
